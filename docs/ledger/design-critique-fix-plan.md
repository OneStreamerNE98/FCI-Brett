...
| A7 — design-system pattern drift | Complete for the critique boundary | PRs #29/#30/#33/#35/#37 establish the structural patterns; PRs #126/#149/#165 consolidate control, card, metric, empty-state, and pill grammar. Read-only `LeadStatusPanel` rows intentionally remain a static list while actionable rows use the shared actionable-list pattern. | Later design enhancements have their own packet status lines in the agent ledger. |
