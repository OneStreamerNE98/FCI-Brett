...
11. **BE-12 complete in PR #53; source-only and undeployed:** rehearsal format v2 now inventories all 24 D1 tables plus R2, keeps every inventory-only category (including the additive AI-01 tasks table and BE-07's four settings/rules/mail categories) fail-closed at zero, and imports clients, contacts, leads, projects, project meetings, and classified activity with identifier/count/content-hash reconciliation. With contiguous production migrations v7–v9 registered, `phone-call` meetings enter the PostgreSQL-facing rehearsal under v8's widened constraint and the three format-v2 flooring KPI project values validate/import/read back under v9; the tasks and BE-07 inventory categories still carry no rows. Full transformation mapping beyond that bounded set, duplicate reporting, backup/restore, cutover, and rollback fixtures remain open. The bounded integration passed only against a disposable GitHub CI PostgreSQL 16 schema; no approved hosted development/staging rehearsal, production migration or grant apply, or live-data operation has run.
...
| 11 | `codex/migration-rehearsal` | **Tracking: BE-12 complete in PR #53:** format-v2 complete source inventory and six-table bounded import are merged source-only and undeployed; complete the remaining transform, duplicate, restore, reconciliation, and cutover tooling/evidence contract | Production-owned schema/routes complete; isolated staging execution requires separate approval |
