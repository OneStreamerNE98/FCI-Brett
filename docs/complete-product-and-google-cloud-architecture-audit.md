...
4. **Completed for the narrow source boundary through PR #55:** the Node/Cloud Run kernel, validated configuration, capped PostgreSQL pools, separate migration/rehearsal commands, health/readiness, secure session/CSRF transport, dashboard/search/project/client/logout routes, fixed administration commands, People projection/page, minimized Activity reader/tab, Workspace OIDC/invitation/session issuance, verifier/attempt-cookie hardening, and the negative-case/real-PostgreSQL login matrix exist without provisioning Google Cloud.
   Draft PR #51 adds client/project creation plus lead and project-meeting reads/creation with the stricter production mutation contract; that extension remains unmerged and undeployed. File/Gmail/Calendar paths remain provider-unavailable; session renewal, the broader employee application and rendered acceptance, live identity configuration/admission, production UI/session composition, PostgreSQL migration/grant apply, deployment, and owner approval remain open.
