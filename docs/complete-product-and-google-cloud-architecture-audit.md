...
11. **BE-12 in progress; source-only:** rehearsal format v2 inventories all 21 D1 tables plus R2, keeps every inventory-only category fail-closed at zero, and imports clients, contacts, leads, projects, project meetings, and classified activity with identifier/count/content-hash reconciliation. Full transformation mapping beyond that bounded set, duplicate reporting, backup/restore, cutover, and rollback fixtures remain open; no rehearsal has been executed.
