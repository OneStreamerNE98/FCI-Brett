...
9. **Durable routes completed in the current source branch:** all nine primary views have fixed URLs with bounded project-status, Settings-section, and Inbox-bucket state plus direct-entry, refresh, history, 404, and denial coverage. Feature splitting and broader typed failure, freshness, conflict, accessibility, and responsive behavior remain open.
...
| 18 | `codex/frontend-durable-routes` then `codex/frontend-quality-hardening` | Durable primary routes and bounded query state implemented in the current branch; feature split, failure/freshness/conflict, and broader accessibility work follows | Core API contracts stable; no hosted configuration change |
