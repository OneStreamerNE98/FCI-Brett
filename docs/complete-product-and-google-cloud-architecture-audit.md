...
The latest controlled deployment remains PR #32 at `adc79b8`, private Sites development version 40, including PR #30's Settings rules semantic table at `aa8ed8f`. The source-only `codex/actionable-lists` slice is complete in PR #33 and is not deployed; the `codex/settings-panel-extraction` SET-01 slice is complete in source in PR #35 and is not deployed. All merged-source packets listed here, including PRs #51, #52, #53, #56, #57, and #66, are likewise source-only and undeployed. Production PostgreSQL migrations 1–6 exist only in source: none has been applied anywhere, and no Cloud SQL instance exists. Live OIDC configuration/admission, session renewal, invitation delivery, production session/CSRF UI composition and broader rendered acceptance, Field Lead link issuance, applying migrations/grants, live storage/integration adapters, provisioning, staging migration/restore, recovery proof, deployment, owner approval, and cutover remain open.
...
- The development D1 model has 23 product and integration tables; the production PostgreSQL registry now defines 28 application tables plus migration history (29 total), with unapproved operational modules explicitly deferred.
