...
9. **Merged through PR #25 and deployed to private Sites development version 37:** all nine primary views have fixed URLs with bounded project-status, Settings-section, and Inbox-bucket state plus direct-entry, refresh, history, 404, denial, responsive, readability, and focused accessibility coverage. Feature splitting, typed failure/freshness/conflict behavior, CSS consolidation, and the remaining interactive-state visual harness remain open.
...
| 18 | `codex/frontend-durable-routes` then `codex/frontend-quality-hardening` | **Merged through PR #25 and deployed to private Sites development version 37:** durable routes, bounded query state, July readability/accessibility states, and focused workflow refinements; shared primitives, feature split, failure/freshness/conflict, and CSS consolidation remain | Core API contracts stable; no hosted configuration or migration change |
