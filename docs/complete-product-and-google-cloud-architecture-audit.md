...
9. **Merged through PR #27 and deployed to private Sites development version 38:** all nine primary views have fixed URLs with bounded project-status, exact Reports lifecycle and Lead-stage, Settings-section, and Inbox-bucket state plus direct-entry, refresh, history, 404, denial, responsive, readability, and focused accessibility coverage. Feature splitting, typed failure/freshness/conflict behavior, CSS consolidation, and the remaining interactive-state visual harness remain open.
...
| 18 | `codex/frontend-durable-routes`, `codex/frontend-quality-hardening`, then `codex/reports-drill-through` | **Merged through PR #27 and deployed to private Sites development version 38:** durable routes, bounded query state including exact Reports lifecycle and Lead-stage filters, July readability/accessibility states, and focused workflow refinements; shared primitives, feature split, failure/freshness/conflict, and CSS consolidation remain | Core API contracts stable; no hosted configuration or migration change |
