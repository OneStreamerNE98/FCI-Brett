...
The current Sites/Workers/D1/R2 application remains useful as a controlled, single-user development environment with test data. It is not yet a production Google Cloud application. Source now includes the PostgreSQL client/project repositories, the generic identity/security-audit/integration/file persistence boundary, a provider-neutral object-storage contract, an owner-approved fail-closed Node/Cloud Run foundation, exact privilege-aware readiness, separate migration/rehearsal commands, least-privilege SQL policy, a bounded core test-data rehearsal, zero-resource-by-default Google Cloud definitions, the approved Administrator/Office/Project Manager/Field Lead policy, source-only employee session transport and route composition, the five fixed Administrator commands, the bounded People projection/page, the minimized Activity reader/tab, durable primary-view routes, the completed July UI critique gap pass, bounded exact Reports lifecycle and Lead-stage drill-through, and the first shared Phase 3 operations UI/filter boundary. Private Sites development version 39 contains those accepted presentation releases. The current Settings rules semantic-table slice remains source-only for review. Durable invitation fulfillment/delivery, production session/CSRF composition, Field Lead link issuance, Workspace OIDC, PostgreSQL migrations 4–5 and reader grants, live storage and integration adapters, provisioning, staging migration/restore, recovery proof, and cutover remain open. Nothing in the source-only production route composition has been applied or deployed to production, and the development adapter does not authorize a second user or real client data.
...
4. **Completed for the narrow source boundary:** the Node/Cloud Run kernel, validated configuration, capped PostgreSQL pools, separate migration/rehearsal commands, health/readiness, secure session/CSRF transport, dashboard/search/project/client/logout routes, fixed administration commands, People projection/page, and minimized Activity reader/tab exist without provisioning Google Cloud. File/Gmail/Calendar paths remain provider-unavailable; the broader employee application, OIDC/session issuance, production UI/session composition, and PostgreSQL apply remain open.
...
9. **Merged through PR #29 and deployed to private Sites development version 39:** all nine primary views have fixed bounded URLs/state and the first shared operations UI/filter boundary, with direct-entry, refresh, history, 404, denial, responsive, readability, and focused accessibility coverage. The current source-only Settings rules slice adds the first reusable responsive semantic table. Actionable-list patterns, feature splitting, typed failure/freshness/conflict behavior, CSS consolidation, and the remaining interactive-state visual harness remain open.
...
| 10 | `codex/admin-field-links` | Separate hashed exact-project Field Link lifecycle and later tab, scheduled with the field-assignment model | Do not reuse file links or create Field Lead users; no live delivery |
| 11 | `codex/migration-rehearsal` | **Partial source evidence exists:** complete transform, duplicate, restore, reconciliation, and cutover tooling/evidence contract | Production-owned schema/routes complete; isolated staging execution requires separate approval |
| 12 | `codex/workspace-oidc-sessions` | Employee-login OIDC and secure-session issuance/renewal; live company client and second-user rollout remain disabled | Recorded staging proof and authorization accepted before live verification |
| 13 | `codex/jobs-scheduler-contracts` | Provider-neutral job/attempt/failure, outbox-relay, future Scheduler/reminder, task-fixture, and replay contracts/tests only | Platform and authorization gates before operational delivery |
| 14 | `codex/google-sync-state-machines` | Gmail and Calendar durable cursors/renewal/reconciliation with fixtures | No live watches/channels |
| 15 | `codex/file-quarantine-contracts` | File metadata, quarantine/scan/release ports and permission tests | Required before untrusted file intake; file policy approved |
| 16 | `codex/http-observability` | Request context, typed errors, headers/limits, structured logs, connector health metrics | Redaction policy approved |
| 17 | `codex/core-record-concurrency` | Edit/archive, atomic conversion, dates/tasks/notes, version conflicts | Authorization enforced |
| 18 | `codex/frontend-durable-routes`, `codex/frontend-quality-hardening`, `codex/reports-drill-through`, then bounded Phase 3 structure slices | **Merged through PR #29 and deployed to private Sites development version 39:** durable routes, bounded query state, July readability/accessibility states, focused workflow refinements, and the first shared operations UI/filter boundary; the current Settings rules semantic table is implemented in source for review; actionable lists, feature split, failure/freshness/conflict, and CSS consolidation remain | Core API contracts stable; no hosted configuration or migration change |
| 19 | Domain branches | Estimate, procurement, schedule/field, messaging, closeout/warranty slices | Each owner decision approved |
