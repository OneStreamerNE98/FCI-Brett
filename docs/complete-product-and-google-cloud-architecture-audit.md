...
9. **Deployed through PR #29 in private Sites development version 39; merged source through PR #30:** all nine primary views have fixed bounded URLs/state and the first shared operations UI/filter boundary, with direct-entry, refresh, history, 404, denial, responsive, readability, and focused accessibility coverage. PR #30 adds the first reusable responsive semantic table for Settings rules in merged source only. Actionable-list patterns, feature splitting, typed failure/freshness/conflict behavior, CSS consolidation, and the remaining interactive-state visual harness remain open.
...
| 18 | `codex/frontend-durable-routes`, `codex/frontend-quality-hardening`, `codex/reports-drill-through`, then bounded Phase 3 structure slices | **Tracking: design-ledger Phase 3 and SET-01–SET-12. Deployed through PR #29 in private Sites development version 39; merged source through PR #30:** durable routes, bounded query state, July readability/accessibility states, focused workflow refinements, the first shared operations UI/filter boundary, and the source-only Settings rules semantic table; actionable lists, feature split, failure/freshness/conflict, and CSS consolidation remain | Core API contracts stable; no hosted configuration or migration change |
