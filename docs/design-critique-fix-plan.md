...
Current source-only follow-ons: `codex/actionable-lists` is complete in PR #33 for the Overview pipeline, Clients, and Projects; Settings extraction/admin gating are complete in PRs #35/#37; the Tier-1 KPI panel is complete in PR #41; and the guided Workspace setup/prerequisites surface is complete in PR #44. None is deployed. KPI-02 is active in unmerged, undeployed draft PR #52 and occupies the sole `FloorOpsApp.tsx` slot, while later extracted Settings packets remain separately bounded.
