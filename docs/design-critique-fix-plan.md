...
Current source-only follow-ons: `codex/actionable-lists` is complete in PR #33 for the Overview pipeline, Clients, and Projects, and it is not deployed. `codex/settings-panel-extraction` completes the Settings-only SET-01 scope in merged PR #35 and is not deployed. SET-02 has passed source acceptance in draft PR #37 with honest Administrator gating and rendered admin/Office coverage; it is also not deployed. KPI-01 takes the next `FloorOpsApp.tsx` slot after PR #37 lands, while later extracted Settings packets remain separately bounded.
