...
- The encrypted, AAD-bound OIDC attempt cookie is stateless and has a fixed ten-minute lifetime; there is no server-side consumed-attempt marker. The router sends a clear-cookie response on every successful or failed callback, and Google authorization codes are single-use. If a browser does not apply that response and retains the attempt cookie, then obtains a fresh code from the same cached authorization URL, reuse remains possible only inside that fixed ten-minute window. That bounded residual is the accepted source-only decision for this small-company rollout; adding durable attempt storage would require a separately reviewed schema, grants, cleanup, and multi-instance lifecycle.
