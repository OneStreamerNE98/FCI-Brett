...
Rendered Playwright runs set `FCI_E2E=true`, which disables Vite environment-file loading and tells the local Cloudflare Worker runtime not to infer variables from `.env.local`, so that file cannot override the synthetic test identities or simulation settings. This does not disable explicit `.dev.vars` bindings.
...
Until native Google Workspace sign-in is implemented, the hosted Sites development environment uses Sign in with ChatGPT for identity and the server-side office allowlist for data access. The accepted production architecture uses Google Cloud with the company's Google Workspace identities and domain restrictions; see `docs/specs/architecture-decision-production-platform.md`, `docs/reviews/complete-product-and-google-cloud-architecture-audit.md`, and `docs/specs/google-cloud-runtime-foundation.md`.
