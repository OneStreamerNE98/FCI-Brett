...
| Durable Inbox analysis and review queue (AI-10 a–e) | Automatic bounded Inbox sweep plus the stored **Needs review** queue; **admin**; `POST`, `GET`, and `PATCH /api/v1/inbox-analysis` | `inboxAnalysis` gates only new sweeps | The client makes no sweep request and POST returns `503 assistant_key_missing`; no provider call or new `mail_items` write occurs. Stored queue rows remain readable and reviewable. | Simulation uses deterministic local fixtures and calls neither Google nor OpenAI. Live sweeps read bounded Gmail evidence and call the provider once per eligible message. Queue GET/PATCH use only the connection-scoped app database. |
...
- **Doc-side pins on THIS spec (know these before editing it):**
  ...
  §11. The same suite pins sentences in `docs/guides/settings-guide.md` and
  `docs/briefs/meeting-notes-and-otter.md`, the `### AI-0N ·` heading grammar and Complete
  statuses for AI-01…09 in the plan ledger (plus a no-Complete pin on AI-10), and the
  assistant scope row in `docs/specs/request-rate-limiting.md`. Prose and pin move together,
