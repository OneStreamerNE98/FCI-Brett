...
- [ ] Split the frontend into durable routes and feature modules with typed errors, query freshness, partial-failure, accessibility, and responsive tests.
  - Durable primary routes, bounded project/Settings/Inbox plus exact Reports lifecycle and Lead-stage query state, direct-entry/refresh/history coverage, route-level denial/404 tests, the July readability/accessibility gap pass, and the first shared operations UI/filter boundary are deployed through PR #29 in private Sites development version 39. PR #30 then merged the Settings rules semantic table source-only at `aa8ed8f`; it has not been deployed. Actionable-list patterns, feature modules, freshness, partial-failure, conflict, CSS consolidation, and the remaining interactive-state visual harness remain.
