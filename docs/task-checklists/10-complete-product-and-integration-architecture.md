...
- [ ] Split the frontend into durable routes and feature modules with typed errors, query freshness, partial-failure, accessibility, and responsive tests.
  - Durable primary routes, bounded project/Settings/Inbox plus exact Reports lifecycle and Lead-stage query state, direct-entry/refresh/history coverage, route-level denial/404 tests, and the July readability/accessibility gap pass are merged through PR #27 and included in private Sites development version 38. Feature modules, freshness, partial-failure, conflict, CSS consolidation, and the remaining interactive-state visual harness remain.
