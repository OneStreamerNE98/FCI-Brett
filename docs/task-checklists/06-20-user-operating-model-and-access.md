...
Recorded here because the EDIT packets deferred to these answers while naming no surface where they would be written. EDIT-05 and EDIT-06 were unbuildable until they existed. Design detail lives in the ledger's [`# Record editing (EDIT)`](../ledger/agent-plan-architecture-workspace-and-setup.md) preamble.
...
The application capability ceiling above is approved, but it does not grant direct Google resource access or make an unimplemented route available. Mailbox reading/delegation, Calendar viewing, Shared Drive root membership, Directory Sheet access, connector administration, job retry, recovery, arbitrary capabilities, per-user exceptions, role-permission toggles, user deletion/re-enablement, and all other deletes remain unavailable in the first release. The accepted production runtime remains insert-only on `audit_events`; the merged Administrator Activity viewer reads only the minimized projection through a separately privileged, security-definer boundary. Its presentation adapter is deployed to private Sites development, but the PostgreSQL projection/grant and Cloud Run reader composition remain unapplied or undeployed. See the [Administration and Access plan](../specs/administration-and-access-plan.md).
