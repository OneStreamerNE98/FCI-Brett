...
- [x] Add a separate general append-only security-audit model for authentication, sessions, roles, permissions, exports, files, connectors, jobs, and recovery. The source model, trigger, atomic repository helper, insert-only runtime grant, and negative tests exist; route-by-route coverage remains part of later authorization/application composition. See [Production persistence boundary](../production-persistence-boundary.md).
