...
- [x] Add a strict bounded core rehearsal harness and safe fixture that preserve test identifiers and verify per-table counts plus content/identifier hashes. BE-12's source-only format v2 inventory classifies all 24 D1 tables plus R2 and expands the bounded import to clients, contacts, leads, projects, project meetings, and classified activity. Every inventory-only category, including the additive AI-01 tasks table and the four BE-07 settings/rules/mail tables, remains fail-closed and zero-only. Production migrations v7–v9 now register those schemas contiguously, so `phone-call` meetings enter the PostgreSQL-facing rehearsal under v8's widened constraint and the three format-v2 flooring KPI values validate, import, read back, and reconcile under v9. The integration passed against a disposable GitHub CI PostgreSQL 16 schema. This is automated source evidence, not the required approved hosted staging or cutover rehearsal below; no production migration/grant apply or live-data operation occurred.
