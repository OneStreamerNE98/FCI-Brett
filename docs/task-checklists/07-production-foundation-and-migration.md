...
- [x] Add a strict bounded core rehearsal harness and safe fixture that preserve test identifiers and verify per-table counts plus content/identifier hashes. BE-12's source-only format v2 inventory classifies all 23 D1 tables plus R2 and expands the bounded import to clients, contacts, leads, projects, project meetings, and classified activity; every inventory-only category remains zero-only. The integration passed against a disposable GitHub CI PostgreSQL 16 schema. This is automated source evidence, not the required approved hosted staging or cutover rehearsal below; no production migration/grant apply or live-data operation occurred.
