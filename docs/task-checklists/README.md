...
| Frontend multi-user hardening | Codex/developer | PR #32 at `adc79b8` is deployed as private Sites development version 40; KPI-02/#52 and later merged UI source remain undeployed | KPI-03 owns the next `FloorOpsApp.tsx` slot; continue the reviewed train with SET-10/#56, then logo refresh/#57. |
...
Status reconciled on July 20, 2026 against merged source baseline `main` at `c2f8b265a7be42f0c091c127923734c7608566bc` and the separate PR #32 deployment baseline at `adc79b855041db04cc3ca2a3eb232bc72408d33b`.
...
PRs #56–#57 remain unmerged and undeployed.
...
| --- | --- | --- |
| [#51](https://github.com/OneStreamerNE98/FCI-Brett/pull/51) | BE-09 production core-record routes | Merged into `main`; source-only and undeployed; BE-10/BE-14 are now assignable. |
| [#52](https://github.com/OneStreamerNE98/FCI-Brett/pull/52) | KPI-02 flooring booking inputs and reports | Merged into `main`; source-only and undeployed; migration 0012 unapplied; KPI-03 now assignable. |
| [#53](https://github.com/OneStreamerNE98/FCI-Brett/pull/53) | BE-12 rehearsal inventory | Merged into `main`; source-only and undeployed; no hosted staging rehearsal, migration apply, or live-data operation occurred. |
...
Continue the reviewed merge order #56 → #57. The still-unclaimed independent packets are coordinated BE-07+SET-05, SET-11, SET-09+WS-10, and WS-13. No checklist item here authorizes a live provider, staging run, migration/apply, production UI composition, owner acceptance, or deployment.
...
Private Sites development version 40 is the latest controlled release. PR #32 merged at `adc79b8`, and that exact deployed commit includes PR #30's semantic-table slice at `aa8ed8f`. Later source through PR #48, documentation reconciliation through PR #50, BE-09 in PR #51, KPI-02 in PR #52, BE-12 in PR #53, OIDC-02/OIDC-03 in PRs #54/#55, and TRK-02 in PR #66 are merged but undeployed.
PRs #56–#57 remain unmerged and undeployed. None of the remaining steps above authorizes production deployment, live OIDC/session admission, staging execution, a migration or infrastructure apply, a second user, or real data.
