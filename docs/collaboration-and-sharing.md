...
Until native Google Workspace sign-in is implemented, the hosted Sites development environment uses Sign in with ChatGPT for identity and the server-side office allowlist for data access. The accepted production architecture uses Google Cloud with the company's Google Workspace identities and domain restrictions; see `docs/architecture-decision-production-platform.md` and `docs/complete-product-and-google-cloud-architecture-audit.md`.
