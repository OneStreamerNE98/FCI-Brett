...
1. **Complete and prove server-enforced identity and authorization.** Source now resolves the approved policy, active/revoked/expired/version-invalid sessions, same-role capabilities, and project-scoped PostgreSQL reads. The Cloud Run entry point source-composes dashboard, search, project list/exact-project, client list, secure transport, and logout; file/Gmail/Calendar routes fail closed as provider-unavailable after authorization. The hosted interface and assistant remain outside that boundary, and no login/session-issuance route exists. Durable invitation/OIDC binding, provider adapters, rendered permission tests, migration/apply, and deployment remain open.
...
5. Implement backup/restore, the separately privileged audit viewer, retention, production login/session issuance and renewal, key rotation, and connector-account continuity. The resolver and source logout route already deny or revoke invalid sessions, but they are not a live login surface. Before accepting untrusted direct uploads or Gmail attachments, also implement file metadata/classification, quarantine, scanning, release, and authorized download controls.
...
4. **Narrow Cloud Run employee routes — implemented in source, not deployed:** PostgreSQL-backed dashboard, search, project list/exact-project, client list, and logout are composed. File/Gmail/Calendar paths are authorization-gated but provider-unavailable; the interface, remaining routes, OIDC/session issuance, and providers remain open.
...
7. **Administration core implemented in source; page and core records next:** follow the simplified [Administration and Access plan](administration-and-access-plan.md). Add the bounded read projection and Management → People & Access with one people/invitation list, read-only role presets, five Administrator workflows, and final-Administrator/session-invalidation safeguards. Add the audit Activity tab before employee rollout and Field Links only with the field-assignment model; continue with safe editing/archive, atomic lead conversion, project dates, tasks/follow-ups, file metadata, notes, activity, and concurrent-write protection.
