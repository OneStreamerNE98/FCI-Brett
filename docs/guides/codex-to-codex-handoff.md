...
PR #30 merged the `codex/semantic-rules-table` assignment into `main` at `aa8ed8f`. It adds one reusable responsive semantic table based on the Access People/Activity pattern, first applied to **Settings → Inbox & file rules**. Native table headings and mobile field labels preserve all five rule fields, while Pause/Enable and Delete remain native keyboard actions. PR #32 then merged at `adc79b8`, and that exact commit was deployed as private Sites development version 40 — the last version-numbered deployment; the owner now deploys on-demand from GitHub (Issue #258) — so the semantic rules table was included in that deployment. The slice changes presentation components, styles, tests, and documentation only; it does not migrate or alter stored records and changes no database schema, API contract, access policy, hosted configuration, migration, or Google connection.
...
The source status is reconciled against merged `main` baseline `599e39f`. PR #51 completed the BE-09 production core-record route packet, PR #53 completed the BE-12 rehearsal inventory packet, PR #52 completed KPI-02 flooring booking inputs and reporting, PR #56 completed SET-10 Workspace connection health, PR #57 merged the reviewed vector logo/app-icon refresh, and PR #66 completed TRK-02 tracking-guard hardening. PRs #63/#64 added the dashboard-driven Workspace setup workstream and PR #65 codified the multi-agent coordination protocol. PRs #54/#55 completed OIDC-02/OIDC-03 in source, PRs #60/#62 reconciled their merged status/guard, and PR #61 expanded the Fable follow-up instructions. The source-only `codex/actionable-lists` slice is complete in PR #33 and is not deployed; the `codex/settings-panel-extraction` SET-01 slice is complete in source in PR #35 and is not deployed. PR #37 added honest authenticated Administrator gating and isolated rendered tests from `.env.local`; PR #41 added the Tier-1 flooring KPI panel and pinned definitions; and PR #44 added the five-step Workspace setup flow plus the hosted-prerequisites table. PRs #52/#56/#57 are likewise source-only and undeployed, and D1 migration 0012 has not been applied to Sites. The last version-numbered deployment was private Sites development version 40 at `adc79b8`; the owner now deploys on-demand from GitHub (Issue #258).
...
**Current status (updated July 22, 2026):** the numbered list below is the mid-July baseline and is retained as history. For current work, the authoritative source is the guard-enforced `Status:` lines in [`agent-plan-architecture-workspace-and-setup.md`](../ledger/agent-plan-architecture-workspace-and-setup.md), with full-review fix packets tracked in [`full-review-2026-07-21-findings.md`](../ledger/full-review-2026-07-21-findings.md). Since that baseline the dashboard-driven Workspace setup wave, flooring KPIs, rate limiting, and the Google-native integration starters have merged source-only and undeployed; the nine-lens full-codebase review and the owner-approved four-stage settings redesign (SET-29–SET-34) are published; and Wave R1 review fixes are complete as of July 22, 2026, with the Wave R2 SET-29 series active. Always read the ledger's wave text for the next open packet rather than this narrative.
...
1. OIDC-01/OIDC-02/OIDC-03 source preconditions are complete in PRs #48/#54/#55; live login remains owner-gated and undeployed.
2. TRK-02 tracking-guard hardening is complete in PR #66 and remains source-only and undeployed.
3. BE-09/#51 is complete in source and merged; it remains undeployed, and BE-10/BE-14 are now assignable.
...
The dated [Claude/Fable review handoff for PRs #51–#57](../archive/pr-51-57-claude-fable-review-handoff.md)
