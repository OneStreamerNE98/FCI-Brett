...
PR #21 merged `codex/admin-audit-viewer` into `main` at `de0fb51`. It implements a separately privileged `GET /api/v1/admin/audit` reader over a security-barrier minimized projection plus an independently loaded **Activity** tab on `/management/access`. It rechecks the exact live Administrator session/version and `audit.read`, accepts only bounded fixed filters and a filter-bound keyset cursor containing a one-way pseudonymous pagination key, and returns only actor, human action, target label, result, bounded friendly reason, and time. Raw audit metadata and internal identifiers remain unavailable. The People/Activity presentation adapter is deployed only to private Sites development version 33. Production migration 5, database grants, employee-session/CSRF composition, live identity, a second user, and real data remain unapplied or disabled.

The current `codex/frontend-durable-routes` branch gives Overview, Leads, Clients, Projects, Schedule, Inbox, Assistant, Reports, and Settings fixed App Router URLs. Project status, Settings section, and Inbox bucket use bounded query parameters; invalid or duplicate route state fails closed to defaults, while free-form search and record drawers remain intentionally transient. Each direct route repeats the server page-shell identity/office check, unknown paths return a real 404, and browser coverage exercises direct entry, refresh, Back/Forward, bookmarks, mobile navigation, and outside-identity denial. This branch changes no database, hosted configuration, Google connection, or deployment.

## Recommended next worker assignments

Review and merge the current `codex/frontend-durable-routes` branch. After that, the next local sequence does not require Brett's Workspace or Cloud input:

1. `codex/frontend-quality-hardening`: finish legacy type/contrast work, broader responsive/accessibility checks, console-error guards, and incremental feature splitting.
