...
The current source-only Cloud Run boundary composes secure session/CSRF transport, authorization, PostgreSQL-backed dashboard/search/project/client/logout paths, and Workspace OIDC initiation/callback with signed-token verification, durable invitation redemption, and secure session-cookie issuance. File list/upload/share, Gmail filing, and Calendar creation remain authorization-gated but return `503 feature_unavailable` because production provider adapters are intentionally absent. It does not issue Field Lead links, seed live users, migrate or apply a database, configure live identity/providers, deploy, add a second user, or use real data.
