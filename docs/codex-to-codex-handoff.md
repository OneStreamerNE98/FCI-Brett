...
> Create and work on `codex/authorization-foundation` after the PostgreSQL repository pull request is reviewed and merged. First read `AGENTS.md`, `docs/20-user-product-and-architecture-review.md`, `docs/architecture-decision-production-platform.md`, `docs/production-postgresql-foundation.md`, `docs/production-postgresql-repositories.md`, and `docs/task-checklists/README.md`. Add forward-only source migrations and provider-neutral contracts for users, invitations, sessions, roles, capabilities, role assignments, and project memberships. Connect stable simulated actor identities to audit, idempotency, and outbox evidence; enforce capabilities and project scope on the server and in data queries; and add denial tests for disabled users, expired or revoked sessions, missing capabilities, outside-domain fixtures, and cross-project access. Preserve the one-user D1/Sites development environment. Do not add live Google login, admit a second user, provision Cloud resources, add credentials, migrate data, connect Workspace, deploy, or merge. Finish with a pull request containing verification evidence and a data/security impact note.
