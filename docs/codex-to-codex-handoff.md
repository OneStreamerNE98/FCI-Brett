...
1. Review and merge the source-only, source-complete `codex/actionable-lists` slice for the whole-row Overview pipeline, Projects, and Clients views. Only after it merges, begin SET-01; continue feature-level client splitting, pill/field/button consolidation, and legacy CSS removal only in later bounded slices.
