...
- Portable repository contracts with D1 and PostgreSQL adapters for Workspace settings, per-user preferences, filing rules, and mail items. Registered production migration v7 defines those four tables, contiguous v8 registers the prepared tasks schema plus the widened `phone-call` meeting constraint, v9 adds the seven flooring KPI project columns and their domain checks, and v10 adds the nullable closed-catalog project segment. Migrations v7–v10 remain source-only and unapplied.
...
The format-version-4 rehearsal snapshot is a strict, test-only exchange format rather than a raw D1 export. It:

- inventories all 27 D1 schema tables plus R2 objects with a reason and one disposition (`migrated`, `transformed`, `excluded`, or `blocking`); the additive AI-01 tasks table and BE-07's four settings/rules/mail categories remain fail-closed and zero-only until the rehearsal format is deliberately expanded; registered v8 now lets `phone-call` meetings enter the PostgreSQL-facing rehearsal under the same widened constraint the target schema applies; a schema-derived test fails if a new D1 table is not classified, and the production runtime does not import the D1 schema;
...
- requires every project to include `flooringCategory`, `squareFeet`, `contractValue`, and `segment`; validates the KPI values against v9 and the nullable closed segment catalog against v10, imports them, reads them back, and includes them in format-v4 SHA-256 evidence;
- requires clients to include `siteAddress`, `latitude`, `longitude`, and `addressValidationVerdict`, and leads/projects to include the three evidence fields alongside their existing `site`; validates the closed saved-verdict and coordinate relationships against v14, preserves permitted legacy null evidence without inventing validation, imports and reads every field back, and includes them in format-v4 SHA-256 evidence;
...
The current rehearsal does **not** migrate generic records, tasks, address-validation review receipts, Gmail archives, Drive mappings, OAuth attempts/tokens, Google connections, Workspace settings, user preferences, filing rules, mail items, R2 objects, or unclassified legacy activity. Format v4 rehearses the three flooring KPI values, nullable project segment, and saved client/lead/project address evidence. Format v3 is rejected before database access rather than silently redefined. The four KPI-03 operational fields are validated through the shared project repository and v9 database constraints rather than added to this bounded snapshot format. Its Google-connection disposition documents separately approved production reauthorization; it does not authorize discarding a nonzero source count or copying credential ciphertext. OAuth and token material must never be exported into a snapshot; production Google authorization will be established again through the approved production connector.
