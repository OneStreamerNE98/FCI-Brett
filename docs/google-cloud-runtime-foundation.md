...
- Portable repository contracts with D1 and PostgreSQL adapters for Workspace settings, per-user preferences, filing rules, and mail items. Registered production migration v7 defines those four tables, and contiguous v8 registers the prepared tasks schema plus the widened `phone-call` meeting constraint. Migrations v7 and v8 remain source-only and unapplied.
