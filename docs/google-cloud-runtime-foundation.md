...
The format-version-3 rehearsal snapshot is a strict, test-only exchange format rather than a raw D1 export. It:

- inventories all 24 D1 schema tables plus R2 objects with a reason and one disposition (`migrated`, `transformed`, `excluded`, or `blocking`); the additive AI-01 tasks table and BE-07's four settings/rules/mail categories remain fail-closed and zero-only until the rehearsal format is deliberately expanded; registered v8 now lets `phone-call` meetings enter the PostgreSQL-facing rehearsal under the same widened constraint the target schema applies; a schema-derived test fails if a new D1 table is not classified, and the production runtime does not import the D1 schema;
...
- requires every project to include `flooringCategory`, `squareFeet`, `contractValue`, and `segment`; validates the KPI values against v9 and the nullable closed segment catalog against v10, imports them, reads them back, and includes them in format-v3 SHA-256 evidence;
...
The current rehearsal does **not** migrate generic records, tasks, Gmail archives, Drive mappings, OAuth attempts/tokens, Google connections, Workspace settings, user preferences, filing rules, mail items, R2 objects, or unclassified legacy activity. Format v3 rehearses the three flooring KPI values plus the nullable project segment, while the four KPI-03 operational fields are validated through the shared project repository and v9 database constraints rather than added to this bounded snapshot format. Its Google-connection disposition documents separately approved production reauthorization; it does not authorize discarding a nonzero source count or copying credential ciphertext. OAuth and token material must never be exported into a snapshot; production Google authorization will be established again through the approved production connector.
