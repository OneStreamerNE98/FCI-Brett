...
- inventories all 24 D1 schema tables plus R2 objects with a reason and one disposition (`migrated`, `transformed`, `excluded`, or `blocking`); the additive AI-01 tasks table remains blocking and zero-only until the rehearsal format is deliberately expanded, a schema-derived test fails if a new D1 table is not classified, and the production runtime does not import the D1 schema;
