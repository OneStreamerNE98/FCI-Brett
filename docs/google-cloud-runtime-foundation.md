...
5. The source-only [production persistence boundary](production-persistence-boundary.md), approved role matrix, narrow employee routes, fixed administration commands, People & Access projection/page, durable invitation fulfillment, OIDC verification, and session issuance are accepted in source. OIDC hardening/test backfill, session renewal, remaining provider adapters, connector-specific credential grants and approved composition, and live configuration/apply then proceed in their gated order.
