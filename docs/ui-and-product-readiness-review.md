...
KPI-02 is complete in merged, undeployed PR #52; migration 0012 is unapplied to Sites, and KPI-03 now owns the next `FloorOpsApp.tsx` queue slot. Later Phase 3 structure slices, provider-neutral job and Gmail/Calendar sync contracts with local fakes, and local migration transformation/reconciliation fixtures also remain available. Private Sites development version 40 remains live; live OIDC admission, staging execution, migration/apply, production deployment, a second user, and real data remain separately gated.
