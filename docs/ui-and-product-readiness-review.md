...
KPI-02 is active in unmerged, undeployed draft PR #52 and is the sole `FloorOpsApp.tsx` queue owner. Later Phase 3 structure slices, provider-neutral job and Gmail/Calendar sync contracts with local fakes, and local migration transformation/reconciliation fixtures also remain available. Private Sites development version 40 remains live; live OIDC admission, staging execution, migration/apply, production deployment, a second user, and real data remain separately gated.
