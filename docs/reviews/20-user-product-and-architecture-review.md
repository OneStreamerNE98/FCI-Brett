...
> detail as historical; the [agent plan](../ledger/agent-plan-architecture-workspace-and-setup.md)
> owns current status.
...
1. Every authenticated user in the hosted development environment still has company-wide data access. The approved source policy now defines granular Administrator, Office, and Project Manager ceilings, exact-one-role/session denial rules, and project-scoped PostgreSQL queries. A narrow Cloud Run source boundary composes dashboard, search, project/client/lead/project-meeting read/write, logout, Workspace OIDC, invitation-redemption, and session-issuance routes, while file/Gmail/Calendar routes are authorization-gated but provider-unavailable. OIDC verifier/attempt-cookie hardening and the negative-case/real-PostgreSQL login test matrix are complete in source in PRs #54/#55. None of this boundary is deployed or connected to the hosted interface; live configuration, migration/grant apply, session renewal, production session/UI composition, and employee admission remain open. The hosted interface still shows only its development `Admin` or `Office` label.
...
2. **Production persistence boundary — complete in source, unapplied:** PostgreSQL migration/repository structures now cover generic identity/security audit and production-owned integration/file metadata, with an opaque provider-neutral object-storage contract. See [Production persistence boundary](../specs/production-persistence-boundary.md).
3. **Authorization policy — implemented in source, not deployed:** the approved granular role ceilings, one-role/no-override rules, invitation/session/Field-link defaults, project scoping, secure-session denial behavior, fixed-operation callback gates, and audit evidence run against accepted production boundaries. See [Authorization simulation](../specs/authorization-simulation.md).
4. **Narrow Cloud Run employee routes — implemented in source, not deployed:** PostgreSQL-backed dashboard, search, project/client/lead/project-meeting read/write, Workspace OIDC/session issuance, administration, and logout are composed. File/Gmail/Calendar paths are authorization-gated but provider-unavailable; the interface, remaining routes, live identity configuration/admission, and providers remain open.
...
6. **Live employee identity:** the Workspace OIDC/invitation/session source and its verifier/attempt-cookie hardening plus negative-case/real-PostgreSQL test matrix are complete in PRs #54/#55. Configure and verify live identity only after the platform, persistence, authorization, and staging gates pass; session renewal, the production interface, migrations/grants, deployment, and owner approval remain open.
7. **Administration and Activity source milestone merged:** the simplified [Administration and Access plan](../specs/administration-and-access-plan.md) now has the bounded People projection/page, one people/invitation list, read-only role presets, five Administrator workflows, final-Administrator/session-invalidation safeguards, and minimized Activity reader/tab. Only the presentation adapter is deployed to private Sites development; production migrations/grants and employee-session/CSRF composition remain unapplied or undeployed. Add Field Links only with the field-assignment model; continue with safe editing/archive, atomic lead conversion, project dates, tasks/follow-ups, file metadata, notes, activity, and concurrent-write protection.
