...
The current Sites/Workers/D1/R2 application remains useful as a controlled, single-user development environment with test data. It is not yet a production Google Cloud application. Source now includes PostgreSQL client/project/lead/project-meeting repositories, the generic identity/security-audit/integration/file persistence boundary, provider-neutral R2/GCS storage adapters, an owner-approved fail-closed Node/Cloud Run foundation, exact privilege-aware readiness, separate migration/rehearsal commands, least-privilege SQL policy, a bounded core test-data rehearsal, default-off deployment definitions, the approved Administrator/Office/Project Manager/Field Lead policy, employee session transport and route composition, Workspace OIDC with durable invitation redemption/session issuance, the five fixed Administrator commands, the bounded People projection/page, the minimized Activity reader/tab, durable primary-view routes, the July UI critique gap pass, Tier-1 flooring KPIs, KPI-02 booking inputs and reports, guided Workspace setup, the Administrator-only persisted Workspace connection-health card (not live provider-health evidence), the approved vector brand assets, and the planned dashboard-driven Workspace setup workstream. Source status is reconciled against merged `main` baseline `599e39f`; PRs #49/#50 completed and guarded the OIDC-04 documentation reconciliation, PRs #54/#55 completed the OIDC verifier/attempt-cookie hardening and negative-case/real-PostgreSQL login test matrix in source, and PRs #60/#62 reconciled their tracking while PR #61 updated the Fable review instructions. PRs #63/#64 added the dashboard-driven setup plan, PR #65 codified the multi-agent coordination protocol, and PR #66 completed TRK-02 tracking-guard hardening.
PR #51 completed the BE-09 production core-record route packet, PR #53 completed the BE-12 rehearsal inventory packet, PR #52 completed KPI-02 flooring booking inputs and reporting, PR #56 completed SET-10 Workspace connection health, and PR #57 completed the application logo asset refresh. All five are merged source-only and undeployed; migration 0012 from PR #52 has not been applied to Sites. The reviewed PR #51–#57 merge train is complete.
The latest controlled deployment remains PR #32 at `adc79b8`, private Sites development version 40, including PR #30's Settings rules semantic table at `aa8ed8f`. The source-only `codex/actionable-lists` slice is complete in PR #33 and is not deployed; the `codex/settings-panel-extraction` SET-01 slice is complete in source in PR #35 and is not deployed. All merged-source packets listed here, including PRs #51, #52, #53, #56, #57, and #66, are likewise source-only and undeployed. Production PostgreSQL migrations 1–10 exist only in source: v7 registers Workspace settings, user preferences, filing rules, and mail items; v8 registers tasks and widens the meeting-type constraint for `phone-call`; v9 adds the seven flooring KPI project fields and their constraints; v10 (`project_segment`, BE-16 / PR #198) adds the project segment column. This enumeration is a dated snapshot — the maintained authority for the current high-water mark is `app/platform/postgres/production-schema-migrations.ts` itself. None has been applied anywhere, and no Cloud SQL instance exists. Live OIDC configuration/admission, session renewal, invitation delivery, production session/CSRF UI composition and broader rendered acceptance, Field Lead link issuance, applying migrations/grants, live storage/integration adapters, provisioning, staging migration/restore, recovery proof, deployment, owner approval, and cutover remain open.
Nothing in the later source train has been applied or deployed, and the development adapter does not authorize a second user or real client data.
...
- The development D1 model has 24 product and integration tables; the production PostgreSQL registry now defines 35 application tables plus migration history (36 total), with unapproved operational modules explicitly deferred.
- Twenty-three application files are coupled to `cloudflare:workers`, and 21 access `env.DB` directly.
- The source-only Cloud Run foundation has a separate container/build, configuration validator, bounded PostgreSQL pool, migration/rehearsal commands, process liveness, exact database/privilege readiness, and a narrow authenticated employee request boundary. Dashboard, search, project/client/lead/project-meeting read/write, administration, and logout routes are composed; authenticated mutations require session-bound CSRF, while the four core-record creation POSTs additionally require idempotency and return the `{data}` envelope. File, Gmail, and Calendar routes fail closed as provider-unavailable after authorization. Source infrastructure definitions exist, but provisioning and the full application port remain open.
- The current allowlist and `isAdmin` flag are appropriate only behind the controlled development host. Durable production user/session/role/membership structures, source-only access-context/query authorization, OIDC/session issuance, verifier/attempt-cookie hardening, and the negative-case/real-PostgreSQL login matrix now exist, and the narrow Cloud Run employee routes use them. The hosted Sites/Workers interface does not; there is no live OIDC configuration or live session issuance.
...
- Gmail suggestions are generated on demand and there is still no durable watch/history processor. AI-10 does now supply a durable review queue: the analysis sweep persists one `mail_items` row per swept message and the Inbox `needs-review` bucket lists those stored rows (PR #235 engine, PR #238 queue), all behind the default-off `inboxAnalysis` toggle and undeployed. Calendar can create an unlinked test hold; it does not yet provide authoritative linked appointments or conflict reconciliation.
...
2. **Completed in source; unapplied:** the production persistence boundary now covers identity, invitation, secure-session, role/capability, project-membership, general security-audit, integration/file metadata, transactional repositories, and an opaque provider-neutral object-storage contract. See [Production persistence boundary](../specs/production-persistence-boundary.md).
3. **Completed in source; not deployed:** the approved first-rollout role ceilings, exact-one-role/no-override rules, secure-session denial behavior, project-scoped PostgreSQL queries, fixed-operation provider callbacks, and negative permission evidence are implemented. See [Authorization simulation](../specs/authorization-simulation.md).
4. **Completed for the current source boundary through PRs #51/#55:** the Node/Cloud Run kernel, validated configuration, capped PostgreSQL pools, separate migration/rehearsal commands, health/readiness, secure session/CSRF transport, dashboard/search/project/client/logout routes, fixed administration commands, People projection/page, minimized Activity reader/tab, Workspace OIDC/invitation/session issuance, verifier/attempt-cookie hardening, and the negative-case/real-PostgreSQL login matrix exist without provisioning Google Cloud.
   Merged PR #51 adds client/project creation plus lead and project-meeting reads/creation with the stricter production mutation contract; that extension remains source-only and undeployed. File/Gmail/Calendar paths remain provider-unavailable; session renewal, the broader employee application and rendered acceptance, live identity configuration/admission, production UI/session composition, PostgreSQL migration/grant apply, deployment, and owner approval remain open.
...
11. **BE-12 complete in PR #53; BE-16 expansion source-only July 25, 2026 and undeployed:** rehearsal format v3 now inventories all 24 D1 tables plus R2, keeps every inventory-only category (including the additive AI-01 tasks table and BE-07's four settings/rules/mail categories) fail-closed at zero, and imports clients, contacts, leads, projects, project meetings, and classified activity with identifier/count/content-hash reconciliation. With contiguous production migrations v7–v10 registered, `phone-call` meetings enter under v8, the three flooring KPI values round-trip under v9, and an explicit closed-catalog project segment survives import/readback/hash reconciliation under v10; the tasks and BE-07 inventory categories still carry no rows. Full transformation mapping beyond that bounded set, duplicate reporting, backup/restore, cutover, and rollback fixtures remain open. Automated integration targets only a disposable GitHub CI PostgreSQL 16 schema; no approved hosted development/staging rehearsal, production migration or grant apply, deployment, or live-data operation has run.
...
| 6 | `codex/cloud-run-employee-routes` | Source-composed dashboard/search/project/client/lead/meeting read/write and logout plus auth-gated provider-unavailable file/Gmail/Calendar paths | No live configuration, migration/apply, or deployment |
...
| 11 | `codex/migration-rehearsal` | **Tracking: BE-12 complete in PR #53; BE-16 format-v3 expansion source-only July 25, 2026:** complete source inventory and six-table bounded import preserve project segment and remain undeployed; complete the remaining transform, duplicate, restore, reconciliation, and cutover tooling/evidence contract | Production-owned schema/routes complete; isolated staging execution requires separate approval |
| 12 | `codex/workspace-oidc-sessions` | **Tracking: BE-04 complete in PR #38; OIDC-01 complete in #48; OIDC-02 complete in PR #54; OIDC-03 complete in PR #55.** Employee-login OIDC, secure-session issuance, verifier/attempt-cookie hardening, and the negative-case/real-PostgreSQL login matrix exist in source; live client/configuration and second-user rollout remain disabled | Complete session renewal, production UI composition and rendered acceptance, then recorded staging proof and authorization acceptance before live verification |
