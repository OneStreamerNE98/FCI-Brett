...
- **Toast overlap** (readiness toast over intro/first row) and **brand-shrink**
  ...
  **Disposition (July 25, 2026, post-#208 flip):** toast-overlap ACCEPTED-AS-IS —
  the toast is a transient auto-dismissing overlay (5s info tier) that obscures no
  content permanently, and FIX-15's scoped suppression (PR #206) now drops this
  exact readiness notice entirely whenever it follows a success within 2s, shrinking
  the observable window further. Brand-shrink ROUTED → DES-10 — wordmark
  size/alignment is precisely DES-10's mockup-first scope; resolving it here would
  preempt the owner's variant sign-off.
