...
The company connector remains separate from employee OIDC identity. A source-only production OAuth workflow now hashes state and browser nonce, encrypts PKCE and refresh-token material with AES-GCM plus purpose-bound AAD, resolves ciphertext by its exact stored key version, and consumes OAuth attempts once through `IntegrationMetadataRepository`. OAuth completion atomically binds verified external identity, the current-version refresh ciphertext, granted scopes, and security-audit evidence in the version-3 tables. Its source-only rotation operation decrypts an exact old version, re-encrypts with the current writer, verifies the new ciphertext before persistence, and applies an exact-version-fenced audited update. WS-17 adds narrowly reviewed column-level connection and credential grants for an explicit Administrator disconnect: the runtime can read only identity/status/version fencing columns, update only revocation/tombstone fields, and append an `integration_events` row. Local credential severance is committed before any provider attempt; the development connector appends the provider outcome separately, while the still-uncomposed production workflow records `not_attempted` until Gate C supplies that post-severance provider composition. It still cannot select credential ciphertext, receives no `integration_connection_scopes` or integration `DELETE` privilege, and production OAuth composition remains gated. Operational integration events remain append-only at the database layer.
...
- no direct runtime access to migration history, credential ciphertext, or connector scopes/cursors; WS-17's explicit disconnect receives only reviewed connection/credential column grants plus append-only `integration_events` `INSERT`;
...
This source boundary was accepted without Brett's open Google Cloud/Workspace inputs because it did not apply or configure production infrastructure. The follow-on [authorization and employee-route work](authorization-simulation.md) now adds explicit access contexts, the approved granular Administrator/Office/Project Manager ceilings, secure session/CSRF behavior, project-scoped queries, fixed-operation provider gates, denial evidence, a narrow dashboard/search/project/client/lead/meeting read/write and logout/login Cloud Run source boundary, the five fixed Administrator commands, the bounded People projection/page, minimized Activity reader, and durable Workspace OIDC invitation/session issuance. The four core-record creation POSTs require idempotency and use the `{data}` envelope. The private Sites development presentation adapter is deployed, but live configuration/admission, production employee-session/CSRF UI composition, provider adapters, the broader application surface, PostgreSQL migration/grant apply, and production deployment remain open.
