...
The PostgreSQL client, project, lead, and project-meeting adapters use one dedicated pooled connection and one short transaction per creation request:
...
5. Insert the client and optional primary contact, the project only when its client exists, the lead, or the meeting only when its project exists.
6. Insert structured append-only activity evidence and the matching `client.created`, `project.created`, `lead.created`, or `project.meeting.created` outbox event.
...
- Project creation persists flooring category, square feet, contract value, and project segment with D1-parity semantics. Segment participates in the idempotency fingerprint; a valid explicit choice wins, while null or a direct-adapter future value derives from the locked parent client's industry.
...
When `TEST_POSTGRES_URL` is present, the PostgreSQL 16 integration suite uses isolated random schemas and covers an eight-request idempotency race, adapter-derived fingerprint and actor/operation scoping, exact deterministic-failure replay, atomic row counts, Unicode duplicates, lead create/read/update/replay, meeting parent validation/create/read/replay, late-transaction rollback, temporary-table shadow resistance on a reused connection, maximum-safe numeric values, versions above JavaScript's safe integer, audited manager assignment, held-lock `SKIP LOCKED` claim and recovery behavior, fenced completion/retry, real terminal-transition rollback, dead-letter audit evidence, and expired-lease recovery. GitHub Actions supplies PostgreSQL 16; normal local tests skip only the real-database cases when no test server is configured.
