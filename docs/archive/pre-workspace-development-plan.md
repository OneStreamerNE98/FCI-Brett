...
- [x] Add the first source-only PostgreSQL core schema and concurrent-runner-safe migration system with immutable checksums, foreign keys, constrained states, timestamps, version columns, idempotency, audit evidence, and an outbox. See [Production PostgreSQL foundation](../specs/production-postgresql-foundation.md).
- [x] Add source-only PostgreSQL client/project adapters, atomic actor-scoped request replay, transactional activity/outbox writes, worker-safe outbox transitions, guarded PostgreSQL value parsing, and PostgreSQL 16 repository coverage. See [Production PostgreSQL repositories](../specs/production-postgresql-repositories.md).
...
15. **Frontend structure worker:** durable URLs, the first shared operations UI/filter boundary, and PR #30's semantic Settings rules table at `aa8ed8f` are included in private Sites development version 40; PR #32 merged at `adc79b8`, and that exact commit was deployed. The source-only `codex/actionable-lists` slice is complete in PR #33 and is not deployed; the `codex/settings-panel-extraction` SET-01 slice is complete in source in PR #35 and is not deployed. Settings admin gating, the Tier-1 KPI panel, and guided Workspace setup are complete in PRs #37/#41/#44 and are also not deployed.
    KPI-02 is complete in merged, undeployed PR #52; migration 0012 is unapplied to Sites, and the `FloorOpsApp.tsx` queue is released to KPI-03. Feature-level component splitting, broader partial-failure/freshness states, and later responsive/accessibility coverage remain.
...
- A strict test-data-only core rehearsal preserves identifiers and verifies counts plus content/identifier hashes inside an isolated non-production schema. BE-12 classifies every D1 table plus R2 and adds bounded lead/project-meeting imports; BE-16 advances the source-only exchange to fail-closed format v3 and preserves project segment under registered PostgreSQL v10. Its integration targets only a disposable GitHub CI PostgreSQL 16 schema and still reports `cutoverReady: false`; as of July 25, 2026, no approved hosted development/staging rehearsal, migration apply, deployment, or live-data operation has run.
- No persistent or hosted Cloud resources, credentials, database roles, migration state, rehearsal data, Workspace connections, hosted configuration, or deployments were created or changed. The CI-only schema and test roles were bounded and removed by the test.

See [Google Cloud runtime foundation](../specs/google-cloud-runtime-foundation.md) for the exact boundary and acceptance gates.
