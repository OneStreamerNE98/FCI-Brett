...
| Settings | Personal settings plus Administrator-only Workspace, calendar, inbox, security, and launch setup | Settings now opens My settings for every Office user, shows the session-provided name and email without duplicating profile data, and keeps per-user timezone, reply signature, and notification choices in the authenticated user's row. The five notification choices remain visibly Planned because no personal delivery consumer exists. Administrators retain the existing Workspace & company setup sections and SET-07 URLs; Office users never render those cards. The fixed-role People & Access page and minimized Activity tab are merged, and their presentation adapter is deployed only to private Sites development. | Connect the planned personal notification catalog only when a delivery consumer is implemented; make saved calendar IDs authoritative at runtime; persist launch checklist state; compose People & Access with the production employee session/CSRF bootstrap; and apply its PostgreSQL migrations/grants. |
...
- CSV preview/import.
- The invitation-redemption, Workspace OIDC/session-issuance boundary, verifier/attempt-cookie hardening, and negative-case/real-PostgreSQL login test matrix are merged in source through PR #55. Session renewal, live identity configuration/admission, PostgreSQL migration/grant apply, production session/UI composition, the broader production interface/routes and rendered acceptance, provider adapters, deployment, and owner approval remain unfinished. The five fixed administration commands, People & Access projection/page, minimized Activity reader/tab, and their rendered permission tests are merged; only their presentation adapter is deployed to private Sites development.
...
See [`architecture-decision-production-platform.md`](../specs/architecture-decision-production-platform.md) for the production boundary and the [Workspace-first, cost-controlled rollout](../specs/architecture-decision-workspace-first-cost-controlled-rollout.md) for the provisioning and cost gates.
...
3. **Complete in source; not deployed:** approved access contexts, capability/project-scoped queries, provider-action gates, negative authorization tests, and narrow dashboard/search/project/client/lead/meeting read/write and logout Cloud Run routes. File/Gmail/Calendar paths are gated but provider-unavailable.
...
6. **Complete in source through PR #55:** Workspace OIDC/session issuance, verifier/attempt-cookie hardening, and the negative-case/real-PostgreSQL login matrix. Next complete session renewal and broader rendered-interface acceptance, then configure and deploy live employee login only after the platform, migration/grant, authorization, and owner gates pass; do not add more users before project permissions are enforced.
...
The owner completed the hosted project-manager correction in private Sites development version 37 on July 18, 2026 and confirmed the corrected identity/activity evidence. PR #29's first Phase 3 shared UI/filter boundary first shipped in version 39. PR #32 then merged at `adc79b8`, and that exact commit deployed as private Sites development version 40, including PR #30's Settings rules semantic-table slice at `aa8ed8f`. The source-only `codex/actionable-lists` slice is complete in PR #33 and is not deployed; the `codex/settings-panel-extraction` SET-01 slice is complete in source in PR #35 and is not deployed. Settings admin gating, the Tier-1 KPI panel, and guided Workspace setup are complete in PRs #37/#41/#44 and are also not deployed.
KPI-02 is complete in merged, undeployed PR #52; migration 0012 is unapplied to Sites, and KPI-03 now owns the next `FloorOpsApp.tsx` queue slot. Later Phase 3 structure slices, provider-neutral job and Gmail/Calendar sync contracts with local fakes, and local migration transformation/reconciliation fixtures also remain available. The last version-numbered deployment was private Sites development version 40 (the owner now deploys on-demand from GitHub, Issue #258); live OIDC admission, staging execution, migration/apply, production deployment, a second user, and real data remain separately gated.
