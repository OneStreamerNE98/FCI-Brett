...
15. **Frontend structure worker:** durable URLs, the first shared operations UI/filter boundary, and PR #30's semantic Settings rules table at `aa8ed8f` are included in private Sites development version 40; PR #32 merged at `adc79b8`, and that exact commit was deployed. The Overview pipeline, Clients, and Projects actionable-list pattern is source-only and complete in PR #33 from `codex/actionable-lists`; it is not deployed. The Settings-only SET-01 extraction is complete in source in PR #35 from `codex/settings-panel-extraction`; it is not deployed. SET-02 has passed source acceptance in draft PR #37 and is not deployed; KPI-01 is next in the `FloorOpsApp.tsx` queue after it lands. Feature-level component splitting, broader partial-failure/freshness states, and later responsive/accessibility coverage remain.
