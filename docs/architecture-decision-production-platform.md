...
4. Extend the narrow source-composed dashboard/search/project/client/logout HTTP boundary across the accepted employee application. Then implement durable explicit-invitation fulfillment, Workspace OIDC verification, production session issuance/renewal, administration APIs, provider adapters, and rendered permissions. The source routes do not themselves admit an employee.
