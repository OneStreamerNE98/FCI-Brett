...
4. Compose the source-only authorization policy and scoped repositories into the production HTTP surface, then implement Workspace OIDC, explicit invitation fulfillment, office-domain verification, production cookie/session endpoints, approved roles/capabilities, and project-level permissions. The local simulation does not itself admit an employee.
