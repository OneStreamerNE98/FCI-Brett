...
4. Extend the narrow source-composed dashboard/search/project/client/logout and fixed administration-command HTTP boundary across the accepted employee application. Then implement the People & Access read projection/page, durable explicit-invitation fulfillment, Workspace OIDC verification, production session issuance/renewal, provider adapters, and rendered permissions. The source routes do not themselves admit an employee.
