...
- **Never two agents in the same files at the same time.** Work is divided by packet:
  ...
  app shell and navigation, Overview, and Reports. Modal/drawer overlays and all extracted
  record surfaces (`LeadsView`, `ClientsView`, `ProjectsView`, and `ScheduleView`) exit the
  queue permanently; work confined to those modules does not claim the
  `app/FloorOpsApp.tsx` slot.
