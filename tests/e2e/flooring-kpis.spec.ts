...
async function mockKpiRecords(page: Page, isAdmin: boolean) {
  ...
  await page.route("**/api/v1/projects", async (route) => {
    if (route.request().method() === "GET") await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ projects }) });
    else await route.continue();
  });
...
test("Tier-1 flooring KPIs render exact formulas, month changes, drill-throughs, and accessible responsive layout", async ({ page }) => {
  ...
  await expect(panel.getByText("$11,667", { exact: true })).toBeVisible();
  await expect(panel.getByText("$51,250", { exact: false })).toBeVisible();
  ...
  const sourceTable = panel.getByRole("table");
  ...
  await expect(panel.getByText("$5,000", { exact: true })).toBeVisible();
...
test("Office users receive counts but no Tier-1 dollar values", async ({ page }) => {
  ...
  await expect(panel.getByText("75%", { exact: true })).toBeVisible();
  await expect(panel.getByText("2 jobs", { exact: true })).toBeVisible();
  await expect(panel.getByText("Administrator only", { exact: true })).toHaveCount(2);
  await expect(panel.getByText("Dollar value available to administrators only", { exact: true })).toHaveCount(3);
  await expect(panel).not.toContainText("$30,000");
  await expect(panel).not.toContainText("$150,000");
  await expect(panel).not.toContainText("$51,250");
