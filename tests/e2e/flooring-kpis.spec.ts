...
const projects = [
  ...
  { id: "kpi-project-completed-july", project_number: "CF-2026-KPI00003", client_id: "e2e-client-001", client_name: "E2E Regression Client", name: "FCI TEST — DO NOT USE — Completed July", status: "completed", site: "Cherry Hill, NJ", project_manager_id: "e2e-admin@example.test", estimated_value: 25_000, flooring_category: "hardwood", square_feet: 1_000, contract_value: 30_000, installation_started_at: Date.parse("2026-07-11T13:00:00Z"), installation_completed_at: Date.parse("2026-07-15T13:00:00Z"), had_callback: 1, callback_note: "FCI TEST — DO NOT USE — Completed July follow-up", created_at: Date.parse("2026-06-01T13:00:00Z"), updated_at: Date.parse("2026-07-15T13:00:00Z") },
  { id: "kpi-project-completed-august", project_number: "CF-2026-KPI00004", client_id: "e2e-client-001", client_name: "E2E Regression Client", name: "FCI TEST — DO NOT USE — Completed August", status: "completed", site: "Cherry Hill, NJ", project_manager_id: "e2e-admin@example.test", estimated_value: 30_000, flooring_category: null, square_feet: null, contract_value: null, installation_started_at: Date.parse("2026-08-10T13:00:00Z"), installation_completed_at: Date.parse("2026-08-15T13:00:00Z"), had_callback: 0, callback_note: null, created_at: Date.parse("2026-08-01T13:00:00Z"), updated_at: Date.parse("2026-08-15T13:00:00Z") },
];
...
test("flooring KPIs render Tier-1 and booking-input formulas, month changes, drill-throughs, and accessible responsive layout", async ({ page }) => {
  ...
  const julyInstallCycleCard = panel.locator(".business-kpi-card").filter({ hasText: "Install cycle · July 2026" });
  await expect(julyInstallCycleCard.getByText("4 days", { exact: true })).toBeVisible();
  await expect(julyInstallCycleCard.getByText("1 completed job with both installation dates", { exact: true })).toBeVisible();
  const julyCallbackRateCard = panel.locator(".business-kpi-card").filter({ hasText: "Callback rate · July 2026" });
  await expect(julyCallbackRateCard.getByText("100%", { exact: true })).toBeVisible();
  await expect(julyCallbackRateCard.getByText("1 recorded Yes across 1 completed · default No can include uncaptured legacy rows", { exact: true })).toBeVisible();
  ...
  await expect(panel.locator(".business-kpi-card").filter({ hasText: "Install cycle · August 2026" }).getByText("5 days", { exact: true })).toBeVisible();
  await expect(panel.locator(".business-kpi-card").filter({ hasText: "Callback rate · August 2026" }).getByText("0%", { exact: true })).toBeVisible();
...
test("Office users receive counts and categories but no flooring dollar values", async ({ page }) => {
  ...
  const estimateAccuracyCard = panel.locator(".business-kpi-card").filter({ hasText: "Estimate accuracy · July 2026" });
  await expect(estimateAccuracyCard.getByText("Contract-value capture details are restricted", { exact: true })).toBeVisible();
  await expect(estimateAccuracyCard).not.toContainText("Not yet captured on booked projects");
