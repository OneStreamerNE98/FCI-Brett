...
test("assistant only promotes an explicitly primary contact", async () => {
  const [evidenceSource, fallbackSource] = await Promise.all([
    read("app/application/assistant/project-evidence.ts"),
    read("app/application/assistant/fallback-answer.ts"),
  ]);

  assert.match(evidenceSource, /const primaryContact = firstContact\?\.is_primary \? firstContact : null/);
  assert.match(fallbackSource, /but none is marked as the primary contact/);
  assert.match(fallbackSource, /Mark one saved client contact as primary/);
  assert.doesNotMatch(evidenceSource, /primaryContact: contacts\.results\[0\] \?\? null/);
});
