...
- Source-only, default-off [Google Chat notifications](docs/specs/google-chat-notifications.md) with typed one-way cards, secret-name-only routing, and simulation audit logging; no webhook is configured and no live message is sent
...
Use the repository ledgers instead of copying a task list here: the [agent execution plan](docs/ledger/agent-plan-architecture-workspace-and-setup.md) is authoritative for active backend, Workspace, and Settings work; the [design-critique remediation plan](docs/ledger/design-critique-fix-plan.md) owns UI remediation; the [owner task checklists](docs/task-checklists/README.md) own setup and acceptance actions; and the [architecture audit roadmap](docs/reviews/complete-product-and-google-cloud-architecture-audit.md#ordered-branch-sized-implementation-roadmap) preserves branch history and gates. The [Pre-Workspace development plan](docs/archive/pre-workspace-development-plan.md) distinguishes work that can proceed locally from work that needs credentials or approval. GitHub issues and pull requests may mirror these ledgers for delivery and review, but they do not create a separate task source of truth.

See the [20-user product and architecture review](docs/reviews/20-user-product-and-architecture-review.md) for the rollout verdict, architecture, access model, priority findings, and corrected delivery order. See [`docs/archive/ui-and-product-readiness-review.md`](docs/archive/ui-and-product-readiness-review.md) (archived) for the detailed page-by-page UI audit.

Development can continue safely before the live Google connection. Follow the [Pre-Workspace development plan](docs/archive/pre-workspace-development-plan.md) for the parallel owner decisions, portable platform work, interface improvements, authorization foundations, and tasks that must wait for Workspace resources or credentials. The completed [portable client and project creation slice](docs/archive/portable-record-creation.md) documents the provider-neutral boundaries and development-environment compatibility. The source-only [production PostgreSQL foundation](docs/specs/production-postgresql-foundation.md), [PostgreSQL repository slice](docs/specs/production-postgresql-repositories.md), and [Google Cloud runtime foundation](docs/specs/google-cloud-runtime-foundation.md) document the constrained core schema, immutable runner, repository/runtime composition, safety controls, and work that still must happen before a complete Cloud SQL cutover.
...
2. Create the `FCI Operations` Shared Drive, `FCI Operations Directory` spreadsheet, `FCI • Client Appointments` calendar, and `FCI • Field Schedule` calendar according to the [Google Workspace organization blueprint](docs/guides/google-workspace-organization.md).
...
See [`docs/guides/google-workspace-rollout-guide.md`](docs/guides/google-workspace-rollout-guide.md) for the exact administrator procedure, runtime values, troubleshooting, and production acceptance checklist. Also see [`docs/briefs/meeting-notes-and-otter.md`](docs/briefs/meeting-notes-and-otter.md) and [`docs/specs/collaboration-and-sharing.md`](docs/specs/collaboration-and-sharing.md).
...
See [`docs/archive/codex-project-handoff.md`](docs/archive/codex-project-handoff.md) (archived) for moving this work between local Codex projects. For coworker collaboration, use the complete [`Codex-to-Codex handoff guide`](docs/guides/codex-to-codex-handoff.md), including GitHub access, cloning, project setup, onboarding prompts, branch workflow, and handback requirements.
