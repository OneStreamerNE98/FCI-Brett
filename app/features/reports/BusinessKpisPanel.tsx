...
export function BusinessKpisPanel({ leads, projects, isAdmin, state, selectedMonth, onSelectedMonthChange }: BusinessKpisPanelProps) {
  ...
  return <section className="panel business-kpis" aria-labelledby="business-kpis-title">
    ...
    <div className="business-kpi-grid" aria-live="polite">
      ...
      <KpiMetric label={`Jobs completed · ${selectedMonthLabel}`} value={ready ? String(kpis.jobsCompleted) : "—"} note={ready ? "Recorded installation completion, then last-update fallback" : pendingNote} icon={CheckCircle2} color="blue" isAdmin={isAdmin} />
      <KpiMetric label={`Install cycle · ${selectedMonthLabel}`} value={!ready || kpis.averageInstallCycleDays === null ? "—" : `${durationFormatter.format(kpis.averageInstallCycleDays)} days`} note={ready ? `${kpis.installCycleJobCount} completed ${kpis.installCycleJobCount === 1 ? "job" : "jobs"} with both installation dates` : pendingNote} icon={Clock3} color="violet" isAdmin={isAdmin} />
      <KpiMetric label={`Callback rate · ${selectedMonthLabel}`} value={!ready || kpis.callbackRate === null ? "—" : rateFormatter.format(kpis.callbackRate)} note={ready ? `${kpis.callbackJobCount} recorded Yes across ${kpis.callbackCompletedJobCount} completed · default No can include uncaptured legacy rows` : pendingNote} icon={RefreshCw} color="orange" isAdmin={isAdmin} />
