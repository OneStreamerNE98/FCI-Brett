...
export async function POST(request: NextRequest) {
  ...
  await env.DB.batch([
    // Development activity rows do not carry a connection key. Simulation-owned
    ...
    env.DB.prepare("DELETE FROM google_form_lead_reviews WHERE connection_key = ?").bind(config.connectionKey),
    env.DB.prepare("DELETE FROM google_form_lead_intake_watermarks WHERE connection_key = ?").bind(config.connectionKey),
    ...
    env.DB.prepare("DELETE FROM workspace_blueprints WHERE connection_key = ?").bind(config.connectionKey),
  ]);
