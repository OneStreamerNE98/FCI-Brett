...
export async function POST(request: NextRequest) {
  ...
  if (!/^multipart\/form-data\b/i.test(contentType) || !/;\s*boundary=(?:"[^"]+"|[^;\s]+)/i.test(contentType)) {
    return noStore({ error: "upload must be valid multipart form data with a boundary" }, { status: 400 });
  }
  const contentLengthHeader = request.headers.get("content-length");
  if (contentLengthHeader !== null) {
    if (!/^\d+$/.test(contentLengthHeader)) {
      return noStore({ error: "content-length must be a non-negative integer" }, { status: 400 });
    }
    const declaredLength = Number(contentLengthHeader);
    if (!Number.isSafeInteger(declaredLength)) {
      return noStore({ error: "content-length must be a safe non-negative integer" }, { status: 400 });
    }
    if (declaredLength > MAX_MULTIPART_BYTES) {
      return noStore({ error: "upload request exceeds the multipart size limit" }, { status: 413 });
    }
  }
  ...
  if (!form) return noStore({ error: "upload must be valid multipart form data" }, { status: 400 });
  const file = form.get("file");
  if (!(file instanceof File)) return noStore({ error: "file is required" }, { status: 400 });
  if (file.size > MAX_FILE_BYTES) return noStore({ error: "file exceeds 20 MB limit" }, { status: 413 });
  const allowed = ["image/jpeg", "image/png", "image/webp", "application/pdf", "text/plain"];
  if (!allowed.includes(file.type)) return noStore({ error: "file type is not allowed" }, { status: 415 });
  if (!await hasAllowedContentSignature(file)) return noStore({ error: "file contents do not match the declared type" }, { status: 415 });
  ...
  if (stored.outcome === "already-exists") {
    return noStore({ error: "upload key already exists; retry the upload" }, { status: 409 });
  }
  return noStore({ key, name: file.name, size: file.size }, { status: 201 });
}
