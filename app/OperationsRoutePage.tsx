...
export async function OperationsRoutePage({ view, returnPath }: { view: OperationsView; returnPath: string }) {
  ...
  return <FloorOpsApp initialView={view} environment={resolveAppEnvironment(process.env.FCI_APP_ENVIRONMENT)} userName={user.displayName} userEmail={user.email} accessLabel={accessLabel} signOutHref={chatGPTSignOutPath(returnPath)} />;
}
